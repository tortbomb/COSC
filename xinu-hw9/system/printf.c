...
uchar getc(void)
{
	...
	wait(serial_port.isema);							//wait for input using input semaphore
	c = (char) serial_port.in[serial_port.istart];		//assign in c the char from istart
	_atomic_decrement(serial_port.icount);				//decrement input buffer count
	serial_port.istart = _atomic_increment(serial_port.istart) % serial_port.icount;	//changing istart
																						//to istart+1 % length
...
syscall putc(char c)
{
	...
	if(serial_port.oidle == 1){			//if idle
		serial_port.oidle == 0;			//make not idle
		//write character to the UART control and status register's data register
	}
